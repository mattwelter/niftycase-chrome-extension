...
const activityFeed = {
  ...
  'natechastain':'0xa3a4548b39da96eb065ff91811ca30da40431c0d', /* $531,425 */
  'OGDfarmer':'0xea39c551834d07ee2ee87f1ceff843c308e089af', /* $153,607 */
  'OnlyToasted':'0x704c978d50590b21e4c2f43eb4da24fe61fcd707', /* $55,691 */
  'pranksy':'0xd387a6e4e84a6c86bd90c158c6028a58cc8ac459', /* $3,526,836 */
  'punk4156':'0xf476cd75be8fdd197ae0b466a2ec2ae44da41897', /* $159,360 */
  'punk6529':'0xfd22004806a6846ea67ad883356be810f0428793', /* $436,886 */
  'Anson_LauHK':'0xa20416801aC2eACf2372e825B4a90ef52490c2Bb', /* $15,000 */
  'xaix2k':'0x2536c09E5F5691498805884fa37811Be3b2BDdb4', /* $1,597,837 - $7.6m */
  'y_kymin':'0xf305F90B19CF66fC2D038f92a26440B66cF858F6', /* $64,196 */
  'daria_daria14':'0x794A816f75f538D5a10437da10422E594966Bb9D', /* $13,000 */
  ...
  elirousso: '0xd8b2a9a6109049ed9b76d787deb6c9cfeb261c3e', /* $300,000 */
...
/* use Element.animate if required */
...
async function getFeed() {
  ...
  const activityFeedCardPlaceholder = 'activityFeedCardPlaceholder';
  ...
  const demoContainerId = 'demoContainer';
  ...
  for (let i=0; i < 10; i++) {
    ...
    const activityFeedCardPlaceholderClass = 'activityFeedCardPlaceholder';
  ...
  const displayProgress = (current, max) => {
    const elem = document.getElementById('myBar');
    const calculatedWidth = (current / max) * 100;
    elem && (elem.style.width = calculatedWidth + '%');
  };
  const activityFeedLen = Object.keys(activityFeed).length;
    ...
    return new Promise(resolve => setTimeout(resolve, i*delay)).then(() => {
      ...
      return fetch(url, options)
        ...
        .catch((error) => {
          ...
          displayProgress(++currentProgress, activityFeedLen);
        });
    });
  }));
...
function initNFTDisplay() {
  ...
  // Remove Profile view to show placeholder or results
...
function displayEvents(events) {
  // Remove Profile view to show results if switched theme too fast
  initNFTDisplay();
  ...
  for (const event of events) {
    ...
    if (eventTypeLabel && eventTypeClass) {
      ...
      const activityFeedCard = 'activityFeedCard';
...
(function() {
  ...
  const initColorMode = (mode) => {
    ...
    switch (mode) {
      case -1:
        ...
        break
      case 0:
        document.body.classList.remove('dark', 'darker');
        document.body.classList.add('light');
        break
      case 1:
        document.body.classList.remove('light', 'darker');
  ...
  const observeCookies = function(callback, interval = 1000) {
    let oldValue = document.cookie;
    setInterval(() => {
      let newValue = document.cookie;
      if (oldValue !== newValue) {
        try {
          // maybe fire event here
          callback({ oldValue: oldValue, newValue: newValue });
        } finally {
          oldValue = newValue;
        }
      }
  ...
  function populateContainer(name, addr) {
    ...
    const dest = document.querySelector(isAuthenticated()
      ? '[aria-label="Profile timelines"]'
      : '[aria-label="Profil-Timelines"]'
    );
    if (!dest) {
      ...
      const frameTempl = templ
        ? templ
        : matchMedia('(prefers-color-scheme: dark)').matches ? 'darker' : 'dark';

      const elem = document.createElement('iframe');
      elem.src = addr
        ? `https://mattwelter.github.io/niftycase/?addr=${addr}&view=2&templ=${frameTempl}` // dark | dm
      ...
      dest && dest.parentElement && dest.parentElement.insertBefore(elem, dest);
    }
  }
  ...
  function populateSidebar(active) {
    ...
    if (!sidebarElement) {
      ...
      var tabImgColor = function() {
        if ((theme === -1 && !matchMedia('(prefers-color-scheme: dark)').matches) || theme === 0)
        {
          return chrome.runtime.getURL(
            active ? 'images/nft_light_filled.png' : 'images/nft_light_outline.png'
          );
        } else if ((theme === -1 && matchMedia('(prefers-color-scheme: dark)').matches) || theme > 0) {
          return chrome.runtime.getURL(
            active ? 'images/nft_dark_filled.png' : 'images/nft_dark_outline.png'
          );
        }
      }
      ...
      //   Light mode icons
      //     https://i.imgur.com/EXHLR3g.png # nft_light_filled.png
      //     https://i.imgur.com/rxOs4c4.png # nft_light_outline.png

      //   Dark mode icons
      //     https://i.imgur.com/UZq5Q0n.png # nft_dark_filled.png
      //     https://i.imgur.com/2A61fXC.png # nft_dark_outline.png

      if (document.querySelector('.nft-tab')) {
        const tabImg = document.getElementById('nftTabImage'),
            tabTxt = document.getElementById('nftTabText'),
            nftTab = document.querySelector('.nft-tab');
        tabTxt.classList.remove('bold');
        nftTab.innerHTML = `
          <div>
            <div><img src=${tabImgColor()} class="nftTabSvg" id="nftTabSvg" /></div>
            <div><span id="nftTabText" class="${active ? 'bold' : ''}">NFTs</span></div>
          </div>
        `;
      } else {
  ...
  chrome.runtime.onMessage.addListener(async (response, sendResponse) => {
    if (response.command === 'load') {
      ...
      if (uri && uri.netloc === 'twitter.com') {
        ...
        const colorMode = getCookies().night_mode;
        initColorMode(colorMode ? parseInt(colorMode) : -1);

        if (!isMainPopulated) populateMain(name);
        populateContainer(name, addr);
        populateSidebar(name === 'nfts');

        // Checks if current user is on their profile page
        var currentUser = document.evaluate('//*[@id="react-root"]/div/div/div[2]/header/div/div/div/div[2]/div/div/div[2]/div/div[2]/div/span', document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.innerHTML.substring(1)
        if (currentUser == name) {
          console.log(currentUser, "is on", "@" + name)
        }
        ...
        window.addEventListener('ColorModeChange', (event) => {
          console.log('color mode changed:', event.detail.mode);
          initColorMode(event.detail.mode);
          if (isMainPopulated) {
            isMainPopulated = false;
            populateMain(name);
          }
  ...
  window.addEventListener('DOMMainLoaded', async (event) => {
    ...
    if (!mainPopulationLock) {
      mainPopulationLock = true;
      await getFeed()
      mainPopulationLock = false;
    }
  });

  observeCookies((details) => {
    const parse = (str) => {
    ...
    const oldValue = parse(details.oldValue), newValue = parse(details.newValue);
    if (oldValue.night_mode !== newValue.night_mode) {
      const mode = newValue.night_mode ? parseInt(newValue.night_mode) : -1;
      fireEvent(window, 'ColorModeChange', { detail: { mode: mode } });
    }
  });
